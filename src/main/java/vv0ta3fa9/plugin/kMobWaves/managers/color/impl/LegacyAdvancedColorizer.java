package vv0ta3fa9.plugin.kMobWaves.managers.color.impl;
...
public class LegacyAdvancedColorizer implements Colorizer {
    ...
    public LegacyAdvancedColorizer(KMobWaves plugin) {
        this.plugin = plugin;
    }
        ...
        for (int index = 0; index < messageChars.length; ) {
            ...
            if (isDoubleTag) {
                ...
                if (isValidColorCharacter(currentChar)) {
                    builder.append(plugin.getUtils().COLOR_CHAR).append(currentChar);
                    index++;
                    continue;
                }
                builder.append('&');
    ...
    private boolean processDoubleTag(StringBuilder builder, char[] messageChars, int index) {
        if (index + 3 <= messageChars.length && isValidHexCode(messageChars, index, 3)) {
            builder.append(plugin.getUtils().COLOR_CHAR).append('x');
            for (int i = index; i < index + 3; i++) {
                builder.append(plugin.getUtils().COLOR_CHAR).append(messageChars[i]).append(plugin.getUtils().COLOR_CHAR).append(messageChars[i]);
            }
            return true;
        }
        return false;
    ...
    private boolean processSingleTag(StringBuilder builder, char[] messageChars, int index) {
        if (index + 6 <= messageChars.length && isValidHexCode(messageChars, index, 6)) {
            ...
            for (int i = index; i < index + 6; i++) {
                builder.append(plugin.getUtils().COLOR_CHAR).append(messageChars[i]);
            }
            return true;
        }
        return false;
