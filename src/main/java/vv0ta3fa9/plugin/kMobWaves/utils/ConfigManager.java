...
public class ConfigManager {
     ...
     * @param coordinates координаты в формате "world:x:y:z"
     * @return true если успешно добавлено, false если уже существует
     */
    public boolean addWaveCoordinates(String coordinates, int i) {
        if (config == null || configFile == null) {
            return false;
        }

        List<String> locations = getStringList("Waves.count" + i + ".coordinates");
        if (locations.contains(coordinates)) {
            return false;
        }
        locations.add(coordinates);
        config.set("Waves.count" + i + ".coordinates", locations);
        try {
